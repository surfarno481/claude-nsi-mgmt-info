...
**Background jobs** (`amiss/job.py`): APScheduler with ThreadPoolExecutor (10 workers). `nsi_poll_dds_job` runs every minute; it wipes the `STP`/`SDP` tables and repopulates them from the **DDS proxy** (`GET /service-termination-points` and `/service-demarcation-points`), setting `isSdpMember=True` on SDP-member STPs. NSI workflow jobs (reserve, commit, provision, release, terminate) are scheduled on-demand per reservation. `nsi_poll_agg_job` fetches reservations from the aggregator proxy and upserts their `Segment`s into the DB, but is **not currently scheduled**.

**NSI integration** (`amiss/nsi.py`, `amiss/dds.py`): SOAP/XML over HTTP with mutual TLS client certificates. `nsi.py` sends NSI commands to the provider. `dds.py` fetches and parses STP/SDP data from the **nsi-dds-proxy** JSON API (`get_dds_proxy_stps`/`get_dds_proxy_sdps` + `dds_proxy_json_to_stps`/`dds_proxy_json_to_sdps`). The legacy raw-DDS NML path (`get_dds_documents`/`unzip`/`to_dict`/`to_list`) is now unused and slated for removal.
